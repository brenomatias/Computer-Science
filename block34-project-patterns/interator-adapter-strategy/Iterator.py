# o problema é o tamanho do que está sendo carregado no servidor.
# Não é possível carregar os 300 GB de dados do banco, então a saída é dividir o resultado da consulta em partes menores, pegando uma de cada vez para alimentar a ferramenta de relatórios.

# implementação de uma classe que faz a simulação do banco de dados:
class DbSimulator:
    def __init__(self):
...
# Para consultá-lo em partes menores, será criada uma classe chamada Iterable
# método __iter__, padronizado pelo Python, responsável por fornecer um objeto iterador
...
# O objeto iterador é uma instância da classe DatabaseIterator, em que será inserida a lógica para acessar o banco de dados e realizar as requisições por lotes (páginas)
...
# como usar? Como vamos iterar na coleção?
